    ...
    for i in range(1, len(lista) - 1):
        if lista[i] != joc.GOL:
            if lista[i - 1] == lista[i] and lista[i + 1] == lista[i]:  
                # Aici se face verificarea propriu zisa. Adica daca sunt
                # 3 * x sau 3 * 0
                return lista[i]
        return False
...
class Joc:
    ...
    def final(self):
      ...
      for i in range(self.NR_COLOANE):
        isFinal = isFinal | elem_identice(self.matr[i * self.NR_COLOANE:((i + 1) * self.NR_COLOANE)])

      for i in range(self.NR_COLOANE):
        coloane = self.matr[i::self.NR_COLOANE]
        isFinal = isFinal | (elem_identice(coloane) != False)

      diag_principala = []
      for i in range(self.NR_COLOANE):
        diag_principala.append(self.matr[i * self.NR_COLOANE + i])

      for sx, sy in [(0, 1), (1, 0)]:
        diag = [self.matr[(sx + i) * self.NR_COLOANE + sy + i] for i in range(3)]
        isFinal = isFinal | (elem_identice(diag) != False)
      isFinal = isFinal | (elem_identice(diag_principala) != False)
      
      diag_secundara = []
      for i in range(self.NR_COLOANE):
          diag_secundara.append(self.matr[i * self.NR_COLOANE + self.NR_COLOANE - i - 1])
			
      for sx, sy in [(2, 0), (3, 1)]:
        diag = [self.matr[(sx - i) * self.NR_COLOANE + sy + i] for i in range(3)]
        isFinal = isFinal | True if elem_identice(diag) else False
			
      isFinal = isFinal | True if elem_identice(diag_secundara) else False
